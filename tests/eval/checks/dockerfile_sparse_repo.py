"""Checks for the dockerfile-sparse-repo scenario.

Fixture: a sparse repo whose strongest signal is a Dockerfile.
"""
...
def check(output: str, generated_dir: Path) -> list[CheckResult]:
    ...
    if inst:
        results.append(hard(
            "References Docker",
            file_contains(generated_dir, inst, "docker"),
        ))
        # Anti-hallucination
        results.append(hard(
            "Did not invent npm/Python/Go/Maven",
            ...
            and file_not_contains_command(generated_dir, inst, "mvn"),
        ))
